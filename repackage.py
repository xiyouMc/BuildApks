...
import getopt, sys, os, zipfile, shutil, traceback, struct, signingapks, time
...
_XIAOYING_APP_KEY = 'yyyyyyyyyyyyyyyy'
def repackage(argv):
    ...
    for channel in options.channels:
        if len(channel.split('&&')[1]) > _CHANNEL_PLACE_HOLDER_LEN:
            raise ValueError('channel string cannot be larger than place-holder:'+ channel.split('&&')[1])
    ...
    for channel in options.channels:
        ...
        channel_val = channelarr[1]
        xiaoying_ak = channelarr[0]
        apkfile = package_channel_apk(raw_axml_data, channel_val, xiaoying_ak, raw_filename, out, temp)
...
def package_channel_apk(raw_axml_data, channel_val, xiaoying_ak, raw_filename, out, temp):
    newapk_name = raw_filename+'-'+channel_val+'-unsigned'
    ...
    replace_axml_string(cloned_buffer, _CHANNEL_PLACE_HOLDER, channel_val)
    replace_axml_string(cloned_buffer, _XIAOYING_APP_KEY, xiaoying_ak)
    temp_manifest = os.path.join(temp, _ANDROID_MANIFEST_XML)
